...
def authenticate(username: str, password: str, role: Optional[str] = None):
    result = False
    query = "select * from users11 where username='{}' and password='{}'".format(
        username, password
    )
...
def new_case(user_info: NewCaseDetail):
    ...
    query = f"insert into submitted_cases11(submitted_by, name, father_name, age, loc,\
...
@app.post("/guest_case")
def guest_case(user_info: NewCaseDetail):
    submitted_by = user_info.submitted_by
    name = user_info.name
    age = user_info.age
    mobile = user_info.mobile
...
def get_training_data(submitted_by: str, status: str = None):
    query = "select case_id, face_encoding from submitted_cases11 where submitted_by='{}'".format(
        submitted_by
    )
    if status:
...
def get_submit_cases():
    query = f"select case_id, face_encoding from submitted_cases11"
    with PostgresConnection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
        return cursor.fetchall()
...
def case_details(case_id: str):
    query = f"select name, father_name, loc,image, mobile, age,updated_on from submitted_cases11 where case_id={case_id}"
    with PostgresConnection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
        return cursor.fetchall()
...
def change_found_status(case_id: str):
    query = f"update submitted_cases11 set status='F' where case_id={case_id}"
    with PostgresConnection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
    return {"status": "success"}
